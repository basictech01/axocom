...
const Chapter5: React.FC = () => {
  ...
  return (
    ...
    <div className="h-[100dvh] w-full bg-[#030303] flex flex-col items-center justify-center font-space overflow-hidden relative selection:bg-primary selection:text-white">
      ...
      <div className="absolute inset-0 z-0 transition-opacity duration-[1500ms] ease-in-out">
        {mediaProperties.map((property, idx) => (
          <div
            key={`bg-${property.id}`}
            className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ease-in-out will-change-[opacity] ${idx === activeIndex ? 'opacity-20' : 'opacity-0'}`}
            style={{ backgroundImage: `url("${property.image}")` }}
          />
        ))}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,transparent_0%,#030303_80%)]"></div>
      ...
      <div className="relative z-20 w-full text-center pointer-events-none drop-shadow-2xl mt-[clamp(1rem,4vh,3rem)] mb-[clamp(1rem,4vh,2rem)] shrink-0">
        <p className="text-primary/80 font-mono text-[clamp(0.65rem,1.2vh,0.875rem)] tracking-[0.3em] uppercase mb-[clamp(0.25rem,1vh,0.5rem)] animate-pulse">
          Cinematic Explorer
        </p>
        <h1 className="text-[clamp(2.5rem,6vh,5rem)] font-work font-black tracking-tighter text-white uppercase leading-[0.9]">
          Our Media Universe
        </h1>
      </div>
